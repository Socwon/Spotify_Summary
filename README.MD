...
이번 프로젝트를 통해 다양한 측면에서 많은 것을 배울 수 있었다. T5 모델을 활용하여 노래 가사로부터 제목을 예측하는 작업은 기술적으로나 이론적으로 매우 흥미로운 도전이었다. 이 작업을 통해 데이터의 품질과 양이 모델 성능에 큰 영향을 미친다는 점을 다시 한 번 깨닫게 되었다. 초기에는 3000개의 샘플로 학습을 진행하였으나 이는 모델의 일반화 능력을 향상시키기에는 다소 부족할 수 있었지만 이후 5000개의 샘플을 분석하면서 더 많은 데이터를 통해 모델이 보다 풍부한 패턴을 학습할 수 있음을 경험하여 데이터의 중요성을 다시 한 번 상기시키는 계기가 되었다. 특히 ROUGE 점수를 통해 모델의 성능을 체계적으로 분석하고, 이를 기반으로 개선 방안을 모색하는 과정이 매우 유익했다. 하이퍼파라미터 튜닝은 모델 성능을 최적화하는 데 중요한 역할을 하며, 작은 변화가 모델의 예측 정확도에 큰 영향을 미칠 수 있음을 배웠다. 이를 통해 모델을 최적화하기 위한 지속적인 실험과 조정의 중요성도 깨달았다. 
또 프로젝트 진행 중 다양한 문제에 직면하게 되었고, 이를 해결하기 위해 여러 방법을 시도하면서 문제 해결 능력이 향상되었다. 예를 들어 코드의 효율성을 높이기 위해 tqdm 라이브러리를 사용하여 진행 상황을 모니터링하고 생성된 결과를 한눈에 보기 쉽게 출력하는 방법을 적용해보았는데 이 경험은 실제 프로젝트 환경에서 문제를 해결하는 데에도 큰 도움이 될 것이다. 
